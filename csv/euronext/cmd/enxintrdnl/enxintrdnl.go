...
func main() {
	...
	if cfg.StartDateDaysBack > 0 {
		sessionDate = sessionDate.AddDate(0, 0, -cfg.StartDateDaysBack)
	}
	log.Printf("days back: %d", cfg.StartDateDaysBack)
	...
	downloadPath := cfg.DownloadsFolder + "intraday/" +
	...
	stati := statistics{
		DownloadErrors: []string{"mep;mic;type;mnemonic;isin;error"},
		FileErrors:     []string{"mep;mic;type;mnemonic;isin;error"},
	}
	...
	if cfg.Concurrency < 2 {
		for i, ins := range instruments {
			ins.download(cfg, i, l, downloadPath, downloadName, &stati)
		}
	} else {
...
func (s *instrument) download(
	...
	insName := s.fileName()
	prefix := fmt.Sprintf("(%d of %d) %s %s: downloading ... ", el+1, elen, insName, s.Type)

	bs, err := intraday.FetchIntradayData(s.Isin, s.Mic, s.Mnemonic, s.Type,
		cfg.DownloadTimeoutDuration, cfg.DownloadRetryDelayDurations, cfg.UserAgent,
		cfg.StartDateDaysBack, cfg.VerboseDownload)
	if err != nil {
		e := err.Error()
	...
	insName += "_" + downloadName

	jsonFile := filepath.Join(downloadFolder, strings.ToUpper(insName)+".json")
